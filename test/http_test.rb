...
describe Down::Http do
  ...
  describe "#download" do
    ...
    it "infers file extension from url" do
      ...
      tempfile = Down::Http.download("#{$httpbin}/robots.txt", extension: "foo")
      assert_equal ".foo", File.extname(tempfile.path)
    end

    it "accepts custom tempfile name" do
      tempfile = Down::Http.download("#{$httpbin}/bytes/100", tempfile_name: "custom-prefix")
      assert_match(/custom-prefix/, tempfile.path)
    end

    it "preserves file extension with custom tempfile name" do
      tempfile = Down::Http.download("#{$httpbin}/robots.txt", tempfile_name: "my-file")
